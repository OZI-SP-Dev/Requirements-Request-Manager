...
export function useEmail(): IEmailSender {
    ...
    const sendSubmitEmail = async (request: IRequirementsRequestCRUD): Promise<void> => {
        ...
        let body = `Hello, a requirement request has been submitted for which you are the approving official by ${request.Requester.Title}.
    ...
    const sendApprovalEmail = async (request: IRequirementsRequestCRUD, comment?: string): Promise<void> => {
        ...
        let body = `Hello, requirement request ${request.getFormattedId()} for ${request.ApplicationNeeded !== ApplicationTypes.OTHER ? request.ApplicationNeeded : request.OtherApplicationNeeded} has been approved by the approving official ${request.Approver.Title}.
    ...
    const sendContractEmail = async (request: IRequirementsRequestCRUD, comment?: string): Promise<void> => {
        ...
        let body = `Hello, requirement request ${request.getFormattedId()} for ${request.ApplicationNeeded !== ApplicationTypes.OTHER ? request.ApplicationNeeded : request.OtherApplicationNeeded} has been put on contract and development for the Requirement will begin soon.
        ${comment ? `The Requirements Manager left a comment saying "${comment}"` : ''}
    ...
    const sendClosedEmail = async (request: IRequirementsRequestCRUD, comment?: string): Promise<void> => {
        ...
        let body = `Hello, requirement request ${request.getFormattedId()} for ${request.ApplicationNeeded !== ApplicationTypes.OTHER ? request.ApplicationNeeded : request.OtherApplicationNeeded} has been marked as Completed and has been Closed by the Requirements Manager ${(await userApi.getCurrentUser()).Title}.
        ${comment ? `The Requirements Manager left a comment saying "${comment}"` : ''}`;
    ...
    const sendCancelledEmail = async (request: IRequirementsRequestCRUD, comment: string): Promise<void> => {
        ...
        let body = `Hello, requirement request ${request.getFormattedId()} for ${request.ApplicationNeeded !== ApplicationTypes.OTHER ? request.ApplicationNeeded : request.OtherApplicationNeeded} has been cancelled by ${currentUser}.
