...
export class RoleDefinitions {
    ...
    private static userIsCito(roles?: RoleType[]): boolean {
        return roles !== undefined && roles.includes(RoleType.CITO);
    }
    ...
    static userCanChangeStatus(request: IRequirementsRequest, newStatus: RequestStatuses | null, currentUser?: IPerson, roles?: RoleType[]): boolean {
        switch (newStatus) {
            ...
            case RequestStatuses.CITO_DISAPPROVED:
                return request.Status === RequestStatuses.ACCEPTED && this.userIsCito(roles);
            case RequestStatuses.REVIEW:
                return request.Status === RequestStatuses.CITO_APPROVED && this.userIsManager(roles);
            ...
            case RequestStatuses.CANCELLED:
                if (request.Status === RequestStatuses.SAVED
                    ...
                    || request.Status === RequestStatuses.DISAPPROVED
                    || request.Status === RequestStatuses.DECLINED
                    || request.Status === RequestStatuses.CITO_DISAPPROVED) {
                    return this.userIsManager(roles)
                ...
                } else if (request.Status === RequestStatuses.ACCEPTED || request.Status === RequestStatuses.CITO_APPROVED || request.Status === RequestStatuses.REVIEW) {
                    return this.userIsManager(roles);
                } else {
                    return false;
                }
